...
class Core_AuthController extends Zend_Controller_Action
{
	...
	function loginAction()
    {
        if (strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
            ...
            if($username == ''){
                $e->add("You must enter a username.");
            }
            if($password == ''){
                $e->add("You must enter a password.");
            }
            ...
            if (!$e->hasErrors()) {        
                ...
                if($result){    
                	...
                	if($url == '' || $url == '/core/auth/login'){
                    	$url = '/admin';
                	}
                }else{
					$e = new DSF_View_Error();
					$e->add('The username or password you entered was not correct.');
					$url = "/core/auth/login";
                }
		...
		if (strtolower($_SERVER["REQUEST_METHOD"]) == "post") {
    		...
    		if($match){
                ...
                {            
    	            ...
    	            $m->add("Your password has been reset for security and sent to your email address");
                }else{
                    $e = new DSF_View_Error();
                    $e->add("Sorry, there was an error sending you your updated password.  Please contact us for more help.");
                }	            
    		}else{
	            $e = new DSF_View_Error();
	            $e->add("Sorry, we could not locate your account. Please contact us to resolve this issue.");
    		}
  		$url =  "/core/auth/login";
		$this->_redirect($url);
